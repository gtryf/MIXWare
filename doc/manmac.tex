% Macros for The TeXbook (slightly modified for our needs)
...
\def\onepageout#1{\shipout\vbox{ % here we define one page of output
    ...
    \vbox to 3pc{ % this part goes on top of the 44pc pages
      \iftitle % the next is used for title pages
        ...
        \ifproofmode\setcornerrules\fi % for camera alignment
...
\newcount\ssecno % for the number of subsubsections in the current section
...
\outer\def\subsection#1. {\medbreak\advance\subsecno by 1\ssecno=0
  \noindent{{\it \chapno.\the\subsecno.}\enspace\ninebf #1}\par\nobreak\noindent}
\outer\def\ssubsection#1. {\medbreak\advance\ssecno by 1
  \noindent{\ninerm\chapno.\the\subsecno.\the\ssecno.\enspace\ninebf #1}\par\nobreak\noindent}
